...
#define itration_thread 43//172 //347
...
__global__ void bw_highpass_filterKernel(float *in, float *output_h,float *out_l)
{
	register float b[FILTER_LENGTH];
	register float a[FILTER_LENGTH];
	b[0] = 1.0000;
	b[1] = -2.0000;
	b[2] = 1.0000;
	a[0] = 1.0000;
	...
	//low
	register float b1[FILTER_LENGTH];
	register float a1[FILTER_LENGTH];
	b1[0] = 1.0000;
	b1[1] = 2.0000;
	b1[2] = 1.0000;
	a1[0] = 1.0000;
		...
		//d1[3] = d1[2];
	{
		...
		output_h[i + j*N_SMPL] = g*b[0] * d[0] + g*b[1] * d[1] + g*b[2] * d[2];


		//low
		/*d1[2] = d1[1];
		d1[1] = d1[0];
		d1[0] = output_h[i + j*N_SMPL] - a1[1] * d1[1] - a1[2] * d1[2];
		out_l[i + j*N_SMPL] = g1*b1[0] * d1[0] + g1*b1[1] * d1[1] + g1*b1[2] * d1[2];*/
...
// try
__global__ void interpolation_kernal(PixelDefine *ptrpixel, float *newscale, PixelDefine *s, BYTE *final_image)
{
//	int	i = threadIdx.y;
	//int b = blockIdx.y;
	...
	if (y*width + x<counter)
	{
		s = ptrpixel+ ptrpixel[y*width + x].offset;
		...
		final_image[ptrpixel[y*width + x].offset] = s->c1 * Q11 + s->c2 *Q21 + s->c3 *Q12 + s->c4 *Q22;
...
// texture
__global__ void interpolation_texture_kernal(PixelDefine *ptrpixel, float *newscale, PixelDefine *s, BYTE *final_image)
{
	...
	if(y*width + x<counter)
	{
		...
		unsigned short Sindx = s->sample_index;
		unsigned short Lindx = s->line_index;
		float u = (float)(Sindx*2+1) / (float)(width*2);
		float v = (float)(Lindx *2+1)/ (float)(height*2);

		final_image[ptrpixel[y*width + x].offset] = tex2D(tex, u, v);  
	}
}
...
{
	dim3 dimBlock(8, 8);
	dim3 dimGrid(width / dimBlock.x, int(height2 / dimBlock.y));
	...
	interpolation_texture_kernal << < dimGrid, dimBlock >> > (pixel, new_scale, s, FI);
...
extern "C" float
dynamic_performance(float *new_scale, float *sampling_out, int *dB_v, float *dynamic, float *max,int dBrange)
{
	dim3 dimBlock(1, no_of_threads);
	dim3 dimGrid(1, no_of_blocks);
	int *dB_p = &dBrange;
